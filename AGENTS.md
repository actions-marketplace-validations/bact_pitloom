...
surfaces (see "Usage surfaces" above). Run these by hand -- or have an
agent run them -- against a real project after any change touching
...
**1. Determinism (bit-for-bit, per "SBOM output" above)**
...
(see "Metadata sources" above). Compare with the same pinned
...
non-CLI entry point** (see "Usage surfaces" above -- exactly the bug
...
suite -- see "Usage surfaces" above): grep each `SKILL.md` for flags and
subcommand names it documents invoking, then confirm each still exists:
