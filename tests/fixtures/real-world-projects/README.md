...
| `uv_build` | [langfuse/langfuse-python](https://github.com/langfuse/langfuse-python) | 4.15.1 | MIT | Confirmed genuine `uv_build` project (`build-backend = "uv_build"`). No static `discover()` module exists for `uv_build` (none is possible -- see `non-hatchling-file-discovery.md`) -- this fixture's `known_gaps: "ALL"` reflects the *default* (non-`--allow-build`) discovery path, which still falls back to the Hatchling heuristic unchanged. Real, `--allow-build`-gated build-and-read discovery for `uv_build` is covered separately, by a slow `@pytest.mark.pypi_network` test (`test_models_wheel_uv_build_real_world.py`), not by this fixture's fast-path `expected.json`. |
| `uv_build` | [rendercv/rendercv](https://github.com/rendercv/rendercv) | 2.8 | MIT | Same as above -- confirmed genuine `uv_build` project, `known_gaps: "ALL"` reflects the default fast-path fallback only. |
| `uv_build` | [uptick/django-model-import](https://github.com/uptick/django-model-import) | 0.9.0 | MIT | More severe uv_build case: `[tool.uv.build-backend] module-name` doesn't match Hatchling's zero-config guess, so the default fallback fails outright with zero files (loudly `WARNING:`-logged, not silent) rather than just over-including -- see `test_default_discovery_fails_loudly_for_module_name_mismatch` (fast, offline) and `backend-file-discovery-validation.md`'s findings. `--allow-build` still reproduces the real wheel exactly. |
