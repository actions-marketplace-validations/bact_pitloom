...
| `uv_build` | [uptick/django-model-import](https://github.com/uptick/django-model-import) | 0.9.0 | MIT | More severe uv_build case: `[tool.uv.build-backend] module-name` doesn't match Hatchling's zero-config guess, so the default fallback fails outright with zero files (loudly `WARNING:`-logged, not silent) rather than just over-including -- see `test_default_discovery_fails_loudly_for_module_name_mismatch` (fast, offline) and `allow-build-validation.md`'s findings. `--allow-build` still reproduces the real wheel exactly. |
