...
_NPY_DIR = Path(__file__).parent.parent.parent / "fixtures" / "aimodels" / "numpy"
...
def test_read_numpy_invalid_file(
    ...
    with patch.dict("sys.modules", {"numpy": mock_np}):
        with caplog.at_level(logging.DEBUG, logger="pitloom.extract.ai_model.numpy"):
            with pytest.raises(ValueError, match="Failed to read NumPy"):
                read_numpy(model_file)
...
def test_numpy_sniff_magic_non_standard_extension(tmp_path: Path) -> None:
    pytest.importorskip("numpy")
    bin_file = tmp_path / "weights.bin"
    bin_file.write_bytes((_NPY_DIR / "example-model-v1.npy").read_bytes())
    meta = read_numpy(bin_file)
    assert meta.format_info.model_format == AiModelFormat.NUMPY
    assert meta.format_info.format_version == "1.0"
