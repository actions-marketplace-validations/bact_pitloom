...
cascade (:mod:`pitloom.extract.lock.cascade`), and real-world
fixture coverage.

See also: test_pipfile.py (core extraction and validation unit
...
REAL_WORLD_LOCKS = (
    Path(__file__).parent.parent.parent / "fixtures" / "real-world-locks" / "pipfile"
)
...
def test_real_world_requests_html() -> None:
    ...
    `setup(name=NAME, ...)`), which `setuptools_py.py`'s literal-only
