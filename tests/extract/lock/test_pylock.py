...
(:mod:`pitloom.extract.lock.pylock`) and its overlay onto
``ProjectMetadata.locked_dependencies`` via ``read_project()``'s lock
cascade (:mod:`pitloom.extract.lock.cascade`).
...
test_poetry.py for the sibling ``poetry.lock`` extractor this
module's tests mirror in shape; test_cascade.py for the
...
from pitloom.extract.lock.pylock import (
...
REAL_WORLD_LOCKS = (
    Path(__file__).parent.parent.parent / "fixtures" / "real-world-locks" / "pylock"
)
