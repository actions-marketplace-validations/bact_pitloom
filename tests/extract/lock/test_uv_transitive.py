...
(:mod:`pitloom.extract.lock.uv`)'s BFS/DFS over a resolved package's own
nested ``dependencies`` list, starting from the root package.

Split out of test_uv.py (which covers this same module's basic
...
per-file line-count guidance -- see test_uv.py's own module
