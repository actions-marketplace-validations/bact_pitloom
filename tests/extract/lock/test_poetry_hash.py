...
def test_filename_matches_version_legacy_sdist_with_hyphen() -> None:
    """Legacy sdists with hyphens (e.g. pytest-cov-2.8.1.tar.gz) must match."""
    assert _filename_matches_version(
        "pytest-cov-2.8.1.tar.gz", "2.8.1", canon_name="pytest-cov"
    )
    assert _filename_matches_version(
        "pytest_cov-2.8.1.tar.gz", "2.8.1", canon_name="pytest-cov"
    )
