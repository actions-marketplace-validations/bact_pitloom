...
policy) and :mod:`pitloom.extract.lock.cascade` (the cascade
...
def extract_pylock_dependencies(project_dir: Path) -> list[str] | None:
    ...
    to a valid-but-empty ``[]``) tells :mod:`pitloom.extract.lock.cascade`'s
    ...
    return resolve_pinned_dependencies(pairs, "pylock.toml")
...
def _pinned_pair_for_package(
    ...
    skip in :func:`pitloom.extract.lock.poetry._main_group_package_or_none`.
