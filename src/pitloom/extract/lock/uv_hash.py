...
:mod:`pitloom.extract.lock.uv`'s ``name==version`` pin extraction.

Split into its own module (rather than growing :mod:`pitloom.extract.lock.uv`
...
duplicating :mod:`pitloom.extract.lock.uv`'s workspace-root
...
from pitloom.extract._extract_utils import filename_from_url
...
def _artifact_hash_candidates(pkg: dict[str, Any]) -> list[tuple[str | None, str]]:
    """Return ``(filename, sha256_digest)`` candidates for one ``[[package]]``
    ...
    for artifact in package_artifacts(pkg):
...
def extract_uv_lock_hashes(
    ...
    :func:`pitloom.extract.lock.uv.extract_uv_lock_dependencies` for the
    same *project_dir*).
